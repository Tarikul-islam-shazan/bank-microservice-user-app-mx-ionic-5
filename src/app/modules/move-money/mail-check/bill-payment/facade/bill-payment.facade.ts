...
export class BillPaymentFacade {
  ...
  private getPaymentSuccessModalCompProps(_paymentInfo: IBillPayment): IMeedModalContent {
    const { amount, executionDate, frequency: paymentFrequency } = _paymentInfo,
      ...
      componentProps: IMeedModalContent = {
        contents: [
          {
            title: this.translate.instant('move-money-module.mail-check.bill-payment.modal.title', {
              paymentAmount,
              payeeName
            }),
            details: [
              this.translate.instant('move-money-module.mail-check.bill-payment.modal.content', {
                paymentFrequency,
                paymentPreposition,
        ...
        actionButtons: [
          {
            text: this.translate.instant('move-money-module.mail-check.bill-payment.buttons.done-button-text'),
        ...
        onDidDismiss: () => this.navigateToPage('/move-money/mail-check')
      };
  ...
  private getEditSuccessCompProp(): IMeedModalContent {
    const componentProps: IMeedModalContent = {
      contents: [
        {
          title: '',
          details: ['move-money-module.mail-check.bill-payment.modal.payment-edit-success-text']
        }
      ],
      actionButtons: [
        {
          text: 'move-money-module.mail-check.bill-payment.buttons.ok-button-text',
          cssClass: 'white-button',
          handler: () => {
            this.analyticsService.logEvent(AnalyticsEventTypes.BillPaymentUpdated);
            this.modalService.close();
            this.navigateToPage('/move-money/mail-check');
          }
        }
      ]
    };
  ...
  private deletePayment(_paymentId: string): void {
    this.payBillService.deletePayment(_paymentId).subscribe(() => {
      this.analyticsService.logEvent(AnalyticsEventTypes.BillPaymentDeleted);
      this.navigateToPage('/move-money/mail-check');
    });
  }
  ...
  private getDeleteCompProp(): IMeedModalContent {
    ...
    const componentProps: IMeedModalContent = {
      contents: [
        {
          details: ['move-money-module.mail-check.bill-payment.modal.payment-delete-text']
        }
      ],
      actionButtons: [
        {
          text: 'move-money-module.mail-check.bill-payment.buttons.yes-button-text',
          cssClass: 'white-button',
          handler: () => {
        ...
        {
          text: 'move-money-module.mail-check.bill-payment.buttons.no-button-text',
